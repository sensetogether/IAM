...
record_aggregation = open(path + "record_interpolation.csv",'a')
...
# Iteration on sizes
for amount in range(10, 300, 10):
    print("Reading amount " + str(amount))

    data_train, data_test = devide_file(path + "DataDay/Data (" + str(day) + ").csv", 1)
    
    start = time.time()
    xyzs_train, values_train = read_whole_data(data_train)
    X_train, M_train = data2tensor(xyzs_train[0:amount], values_train[0:amount], size)
    Xhat, V = tensor_gpr(X_train, M_train)
    end = time.time()
    inter_time = end -start
    print("Interpolation time: ", inter_time, "Aggregation time: ", 0)
    entry = str(amount) + ',' + str(0) + ',' + str(0) + ',' + str(inter_time) + ',' + str(0) + '\n'
