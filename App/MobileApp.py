...
class TestApp(kivy.app.App):
    ...
    def run_aggr(self, btn):
        record_aggregation = open(path + "record_aggregation.csv",'a')
        print("Reading day " + str(day))
        
        data_train, data_test = devide_file(path + "DataDay/Data (" + str(day) + ").csv", 0.7)
        xyzs_train, values_train = read_whole_data(data_train)
        xyzs_test, values_test = read_whole_data(data_test)
        X_test, M_test = data2tensor(xyzs_test, values_test, size)
        ...
        inter_time = inter_time + end - start
        ...
        for pair in it.permutations(ids, 2):        
            ...
            if pair[0] not in aggregated and pair[1] not in aggregated:
                ...
                start = time.time()
                if has_overlap(M1, M2) and compute_cost(X1, M1, X2, M2) < compute_cost(X2, M2, X1, M1):
        ...
        # Iterative aggragation on left
        left = list(set(ids) - set(aggregated))
        ix = ids.index(left[0])
        X, M, n, V = x_list[ix], m_list[ix], n_list[ix], v_list[ix]
        for id_ in left[1:]:
            ix = ids.index(id_)
            print("Aggragating with " + id_)
            X, M, n, V = merge_tensor_gpr(X, M, n, V, x_list[ix], m_list[ix], n_list[ix], v_list[ix])
        end = time.time()
        aggr_time = aggr_time + end - start
        print("Interpolation time: ", inter_time, "Aggregation time: ", aggr_time)
        entry = str(len(values_train)) + ',' + str(len(ids)) + ',' + str(len(aggregated)) + ',' + str(inter_time) + ',' + str(aggr_time) + '\n'
        record_aggregation.write(entry)
        record_aggregation.close()

    def build(self):
        self.lb1 = kivy.uix.label.Label(text="Dataset path: " + path + "Data (" + str(day) + ").csv")
        btn = kivy.uix.button.Button(text="Run aggregation")
        btn.bind(on_press=self.run_aggr)
        self.lb2 = kivy.uix.label.Label(text="Result of aggregation...")

        layout = kivy.uix.boxlayout.BoxLayout(orientation="vertical")
        layout.add_widget(self.lb1)
